...
def train_epoch(loader, model, criterion, optimizer,
    ...
    for i, batch in enumerate(loader):
        ...
        loss, output, stats = criterion(model, target, input_ids1=input_ids1, input_ids2=input_ids2,
        ...
        if not regression:
            ...
            pred = np.argmax(output.detach().cpu().numpy(), axis=0)
            target = target.detach().cpu().numpy()
            target = np.array(list(map(lambda x: label_map[x], target)))
            correct += np.sum(pred == target)
            # pred = torch.argmax(output.data.argmax(1, keepdim=True))
...
def eval(loader, model, criterion, regression=False, verbose=True, eval=True):
    loss_sum = 0.0
    correct = 0.0
    stats_sum = defaultdict(float)
...
def predict(loader, model, eval=True, verbose=True):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            # length = list(map(lambda x:len(x), batch['question']))
            # print('score', score)
            # print('candi', batch['candidate1'])

            # print('question', length)
            # print('input_ids', input_ids)
            # print('attention_masks', attention_mask[0:2, :50])

            # print('sent input_ids', input_ids[0:2,:50])
            # exit()
