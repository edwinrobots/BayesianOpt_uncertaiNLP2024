...
from BNNreward_learner import BNNRewardLearner
...
def process_cmd_line_args(args):
    ...
    elif learner_type_str == 'BNNN':
        if querier_types is None:
            querier_types = ['bnn_random', 'bnn_imp']  # Add BNN-specific query methods if any
        post_weight = 0.3
        n_reps = 1
    ...
    if os.path.exists(reward_file):
        ...
        if querier_type == 'gibbs':
            ...
            querier = ThompsonTopTwoQuerier(learner_type, summary_vectors, heuristics_list, post_weight, n_threads,
                                            rate, lspower)
        elif querier_type == 'bnn_random':
            querier = RandomQuerier(learner_type, summary_vectors, heuristics_list, post_weight, n_threads,
                                    )
        #elif querier_type == 'bnnrandom':
            #querier = BNNQuerier(learner_type, summary_vectors, heuristics_list, post_weight
                                    #)
        else:
        ...
        if learner_type_str != 'H': # heuristics only, no learning
            for round in range(n_inter_rounds):
                ...
                if querier_type != 'random' or round == n_inter_rounds - 1:
                    ...
                    #rewards = querier.getReward([querier.summary_vectors[sum1]], [querier.summary_vectors[sum2]])
                    #print(f"Rewards for summaries {sum1} and {sum2}: {rewards}")
...
def warm_start_with_oracle(learner, dataset, num_topics=10, num_summaries_per_topic=1000, n_threads=0):
    # Initialize preference log
    pref_log = []
    ...
    topic_cnt = 0
    #all_summary_vectors = []
    cumulative_summaries = 0
    # Select a subset of topics
    for topic, docs, models in itertools.islice(data, num_topics):
        # Get summaries and their associated heuristics
        summaries, ref_values_dic, heuristic_list = readSampleSummaries(dataset, topic, feature_type)
        
        # Convert summaries to feature vectors
        summary_vectors = load_summary_vectors(summaries, dataset, topic, root_dir, docs, feature_type)
        #all_summary_vectors.extend(summary_vectors)
        # For each model, get the ROUGE scores and initialize the oracle
        for model in models:
            model_name = model[0].split('/')[-1].strip()
            rouge_values = ref_values_dic[model_name]
            
            # Initialize the oracle with ROUGE scores
            oracle = SimulatedUser(rouge_values, m=temp)
            
            # Generate pairwise preferences using the oracle for the selected subset of summaries
            for i in range(num_summaries_per_topic):
                for j in range(i+1, num_summaries_per_topic):
                    #sum1, sum2 = cumulative_summaries + i, cumulative_summaries + j
                    sum1, sum2 = i, j
                    pref = oracle.getPref(sum1, sum2)
                    pref_log.append([[sum1, sum2], pref])
                    
                    learner.train(pref_log, summary_vectors)
        #cumulative_summaries += len(summary_vectors)

    # Train the model with the initial preferences
    print(type(learner))
    # Train the model with the initial preferences
    #learner.train(pref_log, all_summary_vectors)
...
if __name__ == '__main__':
    ...
    #learner = BNNRewardLearner(input_dim=1024)
    # Call the warm start function
    #warm_start_with_oracle(learner, dataset='DUC2001', n_threads=0)

    # parameters
    if dataset is None:
        dataset = 'DUC2002'  # 'DUC2001'  # DUC2001, DUC2002, 'DUC2004'#
    ...
    chosen_metrics = ['ndcg_at_1%', 'pcc', 'tau', 'ndcg_at_5%', 'ndcg_at_10%', 'rho', 'accuracy']
